define(function(){
    ...
    var ShortestPath = Class.create();
	ShortestPath.prototype = {

    initialize: function(world, start, finish){
    			this.world = world;
    			this.start = start;
    			this.finish = finish;

    			this.start.G=0;
                this.start.H = (Math.abs(world[7][7].Y - world[0][0].Y) + Math.abs(world[7][7].X - world[0][0].X))*10;
                this.start.F = world[0][0].G + world[0][0].H;
                this.finish.H=0;
            ...
            calculatePath: function(){
                ...
                while(this.openList.lenght != 0 || pathNotFound){
                	...
                	if (this.world[this.currentSquare.Y][this.currentSquare.X+1] != undefined){
                		this.caseDroite = this.world[this.currentSquare.Y][this.currentSquare.X+1];
                	}
                	if (this.world[this.currentSquare.Y+1] != undefined){
                		this.caseBas = this.world[this.currentSquare.Y+1][this.currentSquare.X];
                	}
                	if (this.world[this.currentSquare.Y][this.currentSquare.X-1] != undefined){
                		this.caseGauche = this.world[this.currentSquare.Y][this.currentSquare.X-1];
                	}
                	if (this.world[this.currentSquare.Y-1] != undefined){
                		this.caseHaut = this.world[this.currentSquare.Y-1][this.currentSquare.X];
                	}
    ...
    verifyCase: function (caseToCheck){
    ...
    getEveryParentFromCurrentSquare: function (lastSquare){
    ...
    isNotInsideCloseList: function (caseToAnalyse){
    ...
    isNotInsideOpenList: function (caseToAnalyse){
